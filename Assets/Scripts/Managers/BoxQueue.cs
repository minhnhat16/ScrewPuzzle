...
namespace Ingame
{
    public class BoxQueue : SingletonMono<BoxQueue>, IResetable
    {
        ...
        #region Init
        public void Initutorials()
        {
            StartCoroutine(InitTuTo());
             
        }
        public IEnumerator InitTuTo()
        {
            yield return new WaitUntil(() => CameraMain.instance.GetCam() != null);
            yield return new WaitUntil(() => configRecords.Count != 0);

            int totalStar = configRecords.Sum(r => r.NumberOfScrewHoles);
            IngameController.ins.TotalStarInLevel = totalStar;

        
            var reverse = configRecords;
            ConfigStack = new Stack<BoxConfigRecord>(reverse);


            foreach (var config in ConfigStack)
            {
                ColorEnum c = config.BoxColor;

                Debug.Log($"<color=#{c}>■ Box Color {c}</color>");
            }
            InitTutoBox();

        }
        public void Init(bool isTuto)
        {
            if (isTuto)
            {
                Initutorials();
        ...
        public void InitAndShuffleColorSmart()
        {
            ...
            while (true)
            {
                ...
                var color = ChooseNextColor(activeDemand, buckets, last,1);
        ...
        private void InitTutoBox()
        {
            ...
            while (ConfigStack.Count > 0)
            {
                ...
                switch (config.NumberOfScrewHoles)
                {
                    ...
                    default:
                        Debug.LogError($"Unsupported box size: {config.NumberOfScrewHoles}");
                        continue;
                }
    ...
    int maxConsecutive = 1)
        {
            float alpha = 2f;  // ưu tiên theo demand
            float beta = 1f;   // ưu tiên theo design
            ...
            foreach (var kv in buckets)
            {
                ...
                // 🚫 Hard block nếu vượt max liên tiếp
                if (lastColor == color && consecutiveCount >= maxConsecutive)
                    continue;
                ...
                // Soft penalty nếu trùng 1 lần
                if (lastColor == color)
                    penalty = 3f * consecutiveCount;
        ...
        private void CloseAndRemoveBox(
     Box screwBox,
     float time = 0.5f,
     Action onComplete = null
 )
        {
            ...
            screwBox.CloseBox(time, _ =>
            {
                ...
                Debug.Log("[Box] Remaining boxes: " + screwBoxes.Count);

                // 2️⃣ Collect screw FIRST
                MissionManager.ins.ProcessCollectScrew(screwBox.Color, 3);

                // 3️⃣ Process box close mission
                MissionManager.ins.ProcessBoxClosed(screwBox.Color, 1);

                // 4️⃣ Check mission complete
                bool missionComplete = true;

                if (SideMissionManager.ins.currentMission != null)
                {
                    var mission = SideMissionManager.ins.currentMission;
                    missionComplete = mission.currentCount >= mission.requiredCount;
                }

                // 5️⃣ Check last box + mission
        ...
        public void DeactivateAndMoveQueue(Box screwBox, float timeClose = 0.5f)
        {
            ...
            CloseAndRemoveBox(screwBox, timeClose, () =>
            {
                ...
                if (newBox != null)
                {
                    ...
                    TutorialTargetRegistry.Register("box_close", this.transform);
                    TutorialEventBus.Emit(
                    "Screw.MatchCompleted",
                    "blue");
                }
            });
        }
        ...
        public void UnlockedBox()
        {
            ...
            CloseAndRemoveBox(boxUnlock, 0.01f, () =>
            {
                ...
                if (color == ColorEnum.Clear)
                    newBox = TrySpawnNewBox(slot,b => !IsAlreadyHasColor(b)) ;
                else
        ...
        public void AddNewBoxSlot()
        {
            ...
            {
                newBox = TrySpawnNewBox(newBoxSlot, b => b.Color == colorActiveInArray && IsAlreadyHasColor(b));
            }

            if (newBox == null)
            {
                var activesBox = GetActiveColor();

                string activeColorsStr = string.Join(", ", activesBox);

                Debug.Log("Active colors: " + activeColorsStr);

                newBox = TrySpawnNewBox(newBoxSlot, b => !IsAlreadyHasColor(b));
            }
        ...
        private bool IsAlreadyHasColor(Box box)
        {
            var activesColor = GetActiveColor();
            if (activesColor == null) return false;
            return activesColor.Contains(box.Color);
        }

        private HashSet<ColorEnum> GetActiveColor()
        {
            return boxSlots
                .Where(s => s.screwBox != null)
                .Select(s => s.screwBox.Color)
                .ToHashSet();
        }

        private List<Box> GetBoxesInSlots()
        {
            return boxSlots
                .Where(s => s.screwBox != null)
                .Select(s => s.screwBox)
        ...
        #region RenderBoxForTuto
        public void RenderBoxForTuto(ColorEnum color, Vector3 position)
        {
            var box = TrySpawnNewBox(null, b => b.Color == color);
            if (box == null) return;
